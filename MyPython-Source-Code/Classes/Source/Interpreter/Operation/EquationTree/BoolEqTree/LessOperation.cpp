...
const string LessOperation::NAME = "<";

BoolVariable* LessOperation::compareInt(Variable* leftVar, Variable* rightVar)
{
...
BoolVariable * LessOperation::compareFloat(Variable* leftVar, Variable* rightVar)
{
...
LessOperation::LessOperation(unique_ptr<BasicEqTree> left,
                               unique_ptr<BasicEqTree> right) : BooleanEqTree(std::move(left), std::move(right)) { }

Variable* LessOperation::execute(Context& contex)
{
